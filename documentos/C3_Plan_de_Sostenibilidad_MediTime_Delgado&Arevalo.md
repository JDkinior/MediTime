...
```mermaid
graph TD
    UDEC[Universidad de Cundinamarca - Seccional Ubaté] --> GESTORA[Gestora del Conocimiento / Comité Académico]
    GESTORA --> DEV[Equipo Desarrollador e Investigador]
    DEV --> SUP[Soporte Técnico e Infraestructura Cloud]
    DEV --> COM[Capacitación y Comunidad Receptora]
    COM --> PAC[Pacientes y Cuidadores de Ubaté]
    COM --> CLIN[Centros Asistenciales y Red de Salud Local]
```
...
```mermaid
timeline
    title Hoja de Ruta de Evolución Tecnológica de MediTime
    Fase 1 (Meses 1 a 6) : Cierre C3 y Transferencia Territorial : Despliegue de APK y talleres comunitarios en Ubaté : Consolidación del Modo Animales para clínicas veterinarias y mascotas : Monitoreo del asistente Midi y feedback de adherencia
    Fase 2 (Meses 7 a 12) : Integración con Google Health Connect / Apple Health : Reconocimiento de voz local sin dependencia de internet : Módulo de alerta automática por SMS a cuidadores ante dosis omitidas
    Fase 3 (Meses 13 a 24) : Portal Web Clínico para EPS y médicos tratantes : Vinculación con historias clínicas electrónicas : Algoritmos predictivos de desabastecimiento farmacéutico
```
